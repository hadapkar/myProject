...
const COLUMNS = [
  ...
  },
  {
    ...
    fieldName: "isSubscriptionActive",
    type: "boolean"
  },
...
export default class SubscriptionAdmin extends LightningElement {
  ...
  get disableSyncSelected() {
    return (
      !this.selectedRowIds || this.selectedRowIds.length === 0 || this.isLoading
    );
  }

  get disableApplyDate() {
    return this.disableSyncSelected || !this.bulkSubscriptionEndDate;
  }
  ...
  async handleApplyDateToSelected() {
    ...
    try {
      ...
      this.rows = (this.rows || []).map((r) =>
        selected.has(r.id)
          ? { ...r, subscriptionEndDate: this.bulkSubscriptionEndDate }
          : r
      );
      this.draftValues = [];
      await this.refresh();
