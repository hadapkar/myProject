...
export default class FunTargetGame extends LightningElement {
    ...
    get betAmountVisuals() {
        ...
        this._betAmountsCache = BET_NUMBER_BUTTONS
            .filter((slot) => (this.betsByNumber[slot.value] || 0) > 0)
            .map((slot) => ({
    ...
    get totalBetAmount() {
        return Object.values(this.betsByNumber).reduce((sum, amount) => sum + amount, 0);
    }
    ...
    get shouldHighlightBetOk() {
        return this.isBetOkHighlighted && !this.isBetConfirmed && !this.isSpinning && !this.isFinalTenSeconds;
    }

    get isBetOkDisabled() {
        return this.roundPhase !== ROUND_PHASE.BETTING || this.isBetConfirmed || this.totalBetAmount <= 0;
    }
    ...
    spinWheel() {
        const target = this.selectedNumber ?? this.selectedNumbers[this.selectedNumbers.length - 1];
        ...
        this._startSpinAnimation(nextRotation, '2.8s', 'cubic-bezier(0.22, 0.9, 0.26, 1.05)');
    ...
    cancelBet() {
        ...
        const refund = Object.values(this.betsByNumber).reduce((sum, amount) => sum + amount, 0);
        this.coins += refund;
        this.betsByNumber = {};
    ...
    prevBet() {
        ...
        this.selectedNumber = this.selectedNumbers[this.selectedNumbers.length - 1] ?? this._prevBet.number ?? null;
    ...
    disconnectedCallback() {
        ...
        if (this._stateInitialized && this._stateDirty && !this._stateSaveInFlight) {
    ...
    _startAutoSpinRound() {
        ...
        try {
            ...
            const result = predefinedNumber !== null ? predefinedNumber : Math.floor(Math.random() * SEGMENTS);
    ...
    _syncStateFromServer() {
        ...
        getCurrentState()
            .then((state) => {
                ...
                const updatedFrom = String(state.lastUpdatedFrom || '').trim().toLowerCase();
    ...
    _parseBetsJson(value) {
        ...
        try {
            ...
            return Object.keys(parsed).reduce((acc, key) => {
                ...
                if (Number.isInteger(numericKey) && numericKey >= 0 && numericKey <= 9 && numericValue > 0) {
    ...
    _unlockAudio(isMutedWarmup) {
        ...
        const sampleAudio = this._sounds.button || this._sounds.bet || Object.values(this._sounds)[0];
        if (!sampleAudio) {
            return;
        }
    ...
    _betsSignature() {
        return BET_NUMBER_ORDER.map((value) => `${value}:${this.betsByNumber[value] || 0}`).join('|');
    }
